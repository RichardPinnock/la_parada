...
export default function IPVPage() {
  ...
  const fetchIPVData = async (date?: string) => {
    setLoading(true);
    try {
      const url = date ? `/api/ipv?date=${date}` : "/api/ipv";
  ...
  const totalVentas = data.products.reduce(
    (sum, product) => sum + product.T,
    0
  );
              ...
              <TableBody>
                {data.products.map((product, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-right">{product.M}</TableCell>
